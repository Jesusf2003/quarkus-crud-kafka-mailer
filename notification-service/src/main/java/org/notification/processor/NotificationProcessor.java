...
import org.notification.pojo.Payment;
...
public class NotificationProcessor {
	...
	public Uni<Void> process(Record<String, String> record) {
		...
		try {
			...
			// reactiveMailer.send(Mail.withText("govasandovalrosales@gmail.com","Ahoy from
			// Quarkus Reactivo RAA", "A simple email sent from a Quarkus application using
			// the reactive API."));
			log.info("Aqui llegue");
			// mailer.send(Mail.withText("govasandovalrosales@gmail.com", "Ahoy from
			// Quarkus", "A simple email sent from a Quarkus application."));
		} catch (Exception e) {
			throw new IllegalArgumentException(e);
		}
		return reactiveMailer.send( // <4>
				Mail.withText("fernandocanales554@gmail.com",
